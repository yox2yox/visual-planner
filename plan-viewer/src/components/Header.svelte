<script lang="ts">
  import type { EvidenceRef, Metaphor } from '../types'
  ...
  const { title, description, metaphor, takeaway, evidence = [] }: Props = $props()
...
<header class="bg-white border-b border-gray-200 px-6 py-6">
  <div class="max-w-7xl mx-auto">
    ...
    {#if description}
      <p class="mt-2 text-gray-600 text-base max-w-4xl">{description}</p>
    {/if}
      ...
      <div class="mt-5 grid gap-3 md:grid-cols-3">
        {#if metaphor}
          <section class="rounded-lg border border-amber-200 bg-amber-50 p-4">
            ...
            <p class="mt-1 text-sm leading-6 text-gray-700">{metaphor.description}</p>
          </section>
        {/if}
        ...
        {#if takeaway}
          <section class="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <p class="mt-1 text-base font-semibold leading-7 text-gray-900">{takeaway}</p>
          </section>
        {/if}
