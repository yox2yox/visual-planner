...
`unique-id` は同じ plan 内の `glossary[].id` と完全一致する必要があります。クリックすると対象の glossary カードを選択し、必要な親階層を展開してカードまでスクロールします。リンクにマウスを 1 秒間かざし続けると、対象 glossary の説明文をツールチップとして表示します。存在しない ID や空 ID はリンクにならず、表示ラベルだけが通常のテキストとして表示されます。
