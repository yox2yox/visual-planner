...
1. **計画を整理する**。次の要素を特定します。
   ...
   - `glossary[]` — `architectureEdges` のアーキテクチャ図の辺を説明するために必要なノード（アーキテクチャ要素や用語）。
     ...
     - `icon` — その項目固有の絵文字（1 文字）。**type ごとのデフォルト絵文字に頼らず、glossary の各項目には基本的に必ず設定します。** 比喩世界での役回りや責務が一目で伝わる絵文字を選びます（例: 受付係 → 🛎️、記録係 → 📒、案内係 → 🧭、決済処理 → 💳、認証トークン → 🎟️）。同じ計画内でアイコンが衝突しないよう調整します。アイコンを思いつかない汎用ノードに限り省略可（その場合は type のデフォルトが使われます）。
     ...
     - `codeSnippets[]` — **glossary 項目に対応する実コードがリポジトリに存在する場合は必ず設定します。** 関数シグネチャ、クラス/型定義、テーブルスキーマ、リクエスト/レスポンス形状など、その項目を直接説明する短い抜粋（目安 20 行以内）を 1〜2 個入れます。`code`（必須・配列可）に加え、`language`（`ts` / `tsx` / `py` / `sql` など Prism 言語 id）、`label`、`path`、`startLine` / `endLine` を埋めて、読者が出典をすぐ辿れるようにします。長いファイル全体は `codeSnippets` ではなく `evidence` の行参照に回します。スキーマ詳細は `reference/schema.md` の CodeSnippet 節を参照します。
   - 状態レイアウト — 次のどちらか 1 つを選びます。
     ...
     - もし計画ではなく現状を説明するだけの指示を受けた場合は`currentState`を使って説明します。
